	...
	navigator.clipboard.readText().then(text=>{ // Формирование рекламы при вставке ссылки на страницу с продуктом
		if(text.trim().match(/^\d{6}$/)&&!$('[contenteditable]:focus').length){
			...
			$.post(link,function(data){
				...
				until=data.split('saleUntilDate&quot;:&quot;')[1].split('T23:59:59+')[0].match(/(\d{4}-\d{2}-\d{2})/g)[0];
				if(until!=null){
				...
				if(sendData.description.length<120){ // Если слишком короткое описание, то рассмотрим первый абзац полного описания продукта
					...
					sendData.description=sendData.description.replace(/\&amp\;/g,'&').replace(/\&bull\;/g,'•').replace(/\.([А-ЯA-Z0-9\«])/g,'<br>$1').replace(/(\.|)(\•)/g,'<br>$2').replace(/([…!🙂])([А-ЯA-Z0-9])/g,'$1<br>$2').trim().replace(/\.$/gm,'');
					if(sendData.description.length<120){ // Если слишком короткий первый абзац полного описания продукта, то рассмотрим описание полностью

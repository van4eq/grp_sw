	...
	navigator.clipboard.readText().then(text=>{ // Формирование рекламы при вставке ссылки на страницу с продуктом
		...
		if(text.trim().match(/^\d{6}$/)){
			$.post(link,function(data){
				...
				sendData.description=sendData.description.replace(/\&amp\;bull\;/g,'•').replace(/\.([А-ЯA-Z0-9\•\«])/g,'<br>$1').replace(/([…!🙂])([А-ЯA-Z0-9])/g,'$1<br>$2').trim().replace(/\.$/gm,'');
...
$('#copy button').click(function(){
	...
	if($('#remain+label').text().match(/\d+/)){
		remain=bold+declension($('#remain+label').text().match(/\d+/),new Array('штука','штуки','штук'),new Array('Осталась','Осталось','Осталось'))+bold+'\n\n';
	}
	var addLink='';
